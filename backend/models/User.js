...
const userSchema = new mongoose.Schema(
  {
    ...
    // Doctor fields
    medicalLicense: { type: String, trim: true, maxlength: 100 },
    ...
    dateOfBirth: {
      type: Date,
      required: function () {
        return this.role === 'patient' && !this.googleId;
      }
    },
    address: {
      type: String,
...
userSchema.virtual('age').get(function () {
  return this.dateOfBirth
    ? Math.floor((Date.now() - this.dateOfBirth) / (365.25 * 24 * 60 * 60 * 1000))
    : null;
});

userSchema.virtual('bmi').get(function () {
  return this.height && this.weight
    ? Math.round((this.weight / ((this.height / 100) ** 2)) * 10) / 10
    : null;
});
